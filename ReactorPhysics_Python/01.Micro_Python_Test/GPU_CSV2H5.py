...
def extract_mf3(mt, ntt, m):
    ...
    if iRowFound > 0:  # there is mf=3 and required mt for this isotope
        ...
        iRow = iRowFound + 1
        enGroup = int(m[2, 2])
        sig = torch.zeros((nSig0, enGroup), dtype=torch.float32)
        while m[iRow, 7] == 3 and m[iRow, 8] == mt:
            ig = int(m[iRow-1, 5])
            a, iRowNew = extractNwords(nSig0 * nLgn * 2, iRow, m)
            sig[0:nSig0, ig-1] = torch.from_numpy(a[nSig0*nLgn:(nSig0*nLgn+nSig0)])
            #sig.append(a[nSig0*nLgn:(nSig0*nLgn+nSig0)])
            iRow = iRowNew + 2                                       
    else:
        #sig.append(0)
        sig = torch.zeros(1, dtype=torch.float32)
                ...
                for iTo in range(ig2lo, ig2lo + ng2 - 1):
                    ...
                    for iSig0 in range(int(nSig0)):
                        for iLgn in range(int(nLgn)):
                            k += 1
                            data = torch.cat((torch.tensor([iLgn, iSig0, nonz - 1]), torch.unsqueeze(torch.tensor(a[k-1]), 0)))
                            sig.append(data)

        iRow += 1

    if nTemp == -1:
        sigFinal = torch.zeros(1)
    else:
        sig = torch.stack(sig)
        z = sig.shape[0] // (nLgn * nSig0)
        sigFinal = sig[:, -1].reshape((nLgn, nSig0, z))
        if nLgn == 1:
            rows_of_zeros = torch.zeros((2, nSig0, z))
            sigFinal = torch.cat((sigFinal, rows_of_zeros), dim=0)

    ifrom = torch.tensor(ifrom)
    ito = torch.tensor(ito)
        ...
        for iTemp in range(nTemp): # loop over temperatures
            ...
            if not os.path.exists(isoName + '.h5'): # if the corresponding HDF5 file does not exist
                ...
                with h5py.File(isoName + '.h5', 'w') as hdf:
                    ...
                    sigS = [[torch.zeros((ng, ng)) for _ in range(nSig0)] for _ in range(nLgn+1)]
                    ...
                    if torch.all(sigF == 0):
                        ...
                        #=====================================================================
                        nubar = torch.zeros((nSig0,ng))
                        nubar_G.create_dataset('nubar', data = nubar)
                        # fission spectrum
                        chi = torch.zeros((nSig0,ng))
