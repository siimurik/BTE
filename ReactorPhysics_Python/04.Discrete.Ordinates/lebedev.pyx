...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef dict getLebedevSphere(int degree):
    # Function implementation (as shown in the previous response)

    # Define the dictionary with NumPy arrays
    cdef dict leb_tmp = {
        'x': np.zeros(degree, dtype=np.float64),
        'y': np.zeros(degree, dtype=np.float64),
        'z': np.zeros(degree, dtype=np.float64),
